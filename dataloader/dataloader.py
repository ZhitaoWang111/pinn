...
class DF():
    ...
    def read_one_csv(self,file_name,nominal_capacity=None, nominal_rul=None):
        '''
        read a csv file and return a DataFrame
        :param file_name: str
        :return: DataFrame
    ...
    def load_one_battery(self,path,nominal_capacity=None, nominal_rul=None):
        '''
        ...
        :return:
        '''
        df = self.read_one_csv(path,nominal_capacity, nominal_rul)
        x = df.iloc[:,:-2].values
        y = df.iloc[:,-2:].values
        x1 = x[:-1]
        x2 = x[1:]
    ...
    def load_all_battery(self,path_list,nominal_capacity, nominal_rul):
        '''
        ...
        for path in path_list:
            (x1, y1), (x2, y2) = self.load_one_battery(path, nominal_capacity, nominal_rul)
            # print(path)
        ...
        train_X1, valid_X1, train_X2, valid_X2, train_Y1, valid_Y1, train_Y2, valid_Y2 = \
            train_test_split(train_X1, train_X2, train_Y1, train_Y2, test_size=0.2, random_state=420)  # random_state随机种子 
...
class XJTUdata(DF):
    def __init__(self, root, args):
        ...
        self.variables = pd.read_csv(os.path.join(root, self.file_list[0])).columns     # 各列的变量名
        self.num = len(self.file_list)
        self.batch_names = ['2C','3C','R2.5','R3','RW','satellite']
        self.batch_size = args.batch_size

        if self.normalization:      # 设置归一化的话，设置标称容量为 2
            self.nominal_capacity = 2.0
            self.nominal_rul=None
        else:
...
class HUSTdata(DF):
    def __init__(self,root='../data/HUST data',args=None):
        ...
        if self.normalization:
            self.nominal_capacity = 1.19
            self.nominal_rul = 3000
        else:
            self.nominal_capacity = None
            self.nominal_rul = None
    ...
    def read_all(self,specific_path_list=None):
        ...
        if specific_path_list is None:
            ...
            for file in files:
                path = os.path.join(self.root,file)
                file_list.append(path)
            return self.load_all_battery(path_list=file_list, nominal_capacity=self.nominal_capacity, nominal_rul=self.nominal_rul)
        else:
            return self.load_all_battery(path_list=specific_path_list, nominal_capacity=self.nominal_capacity, nominal_rul=self.nominal_rul)
